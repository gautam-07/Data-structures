/*
*******************************************************
✥ Title  :- Dynamic Array Data Structure
...
// Declaring and defining two more functions to make it dynamic.
...
// Double the array after original array is at condition of overflowing
void DoubleArray(Array* arr)
{
   int *temp,i;
   temp = new int[arr->capacity*2];
   for(i=0;i<=arr->lastIndex;i++)
   {
      temp[i] = arr->ptr[i];
   }
...
// Half the array if data items are less than half the size of original array.
void HalfArray(Array* arr)
{
   int *temp,i;
   temp = new int[arr->capacity/2];
   for(i=0;i<=arr->lastIndex;i++)
